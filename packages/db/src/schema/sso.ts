// 企业 SSO + 密钥实体(见 08 章 16):sso_connections / cert_store / instance_signing_keys / tenant_signing_keys /
...
// 16.4 tenant_signing_keys(已废弃保留表;私钥信封加密拆三 blob)
export const tenantSigningKeys = sqliteTable(
  ...
  (t) => [
    uniqueIndex('tenant_signing_keys_tenant_kid_unq').on(t.tenantId, t.kid),
    index('tenant_signing_keys_tenant_status_idx').on(t.tenantId, t.status),
  ],
)
