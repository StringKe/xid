...
describe('verifyAuthentication: valid path', () => {
  ...
  // 回归(删除长度短路):本包契约 ES256 签名只接受 DER,始终走 derToP1363。
  // DER 签名验签通过;同一签名的 raw P1363(64 字节)形式不再被短路放行,derToP1363 视其为畸形 DER 拒绝。
  // 这保证真实 64 字节 DER 不会被长度启发式误当 P1363(原 bug:约百分之几合法登录偶发失败)。
  it('verifies the DER form but rejects the raw P1363 form of the same signature', async () => {
    ...
    // 把 DER 转回 raw P1363(64 字节),冒充已转换输入。删除短路后必须不被放行。
