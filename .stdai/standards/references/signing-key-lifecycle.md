...
SAML signing keys are kept separate from OIDC signing keys, in the `cert_store` table (`packages/db/src/schema/sso.ts`), using the same three-blob envelope encryption structure. `usage` distinguishes `saml_sp_signing` / `saml_sp_encryption` / `saml_idp_signing`, and `status` keeps old and new certificates alive side by side during rotation. See `docs/design/04-enterprise-sso.md`.
