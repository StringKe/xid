...
- `tenant_signing_keys` is a deprecated reserved table. TenantContext resolution loads only `instance_signing_keys` (`packages/db/src/tenant-context.ts`). It MUST NOT become the default signing source for `xid.dev`.
