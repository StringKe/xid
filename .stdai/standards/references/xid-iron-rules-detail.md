...
4. **Signing key isolation**: the default issuer signs with a per-instance ES256 key,
   ...
   briefly inside the isolate and are NEVER persisted in the clear. `tenant_signing_keys` is a
   deprecated retained table kept for early per-org-issuer migration and a future explicit custom
   issuer; it MUST NOT act as the default signing source for the instance issuer.
