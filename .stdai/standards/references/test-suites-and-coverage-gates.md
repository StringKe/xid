...
description: How tests run in each XID workspace (the three apps, vp packages, root contract gates) and the exact coverage floor numbers for the core and worker gates
---
...
- `apps/server` runs vitest directly with two configs: `vitest run --config vitest.worker.config.ts && vitest run --config vitest.spa.config.ts`. The SPA project covers Hosted Auth and account only.
- `apps/site` owns Nimbus/Astro checks, Site Worker tests, generated-content contracts, Pagefind and
  final `dist` route audits. Site tests MUST prove all 8 locales and all public docs are present in
  initial HTML and generated Markdown.
- `apps/console` owns Console React tests, Console Worker tests and apex/tenant-host navigation
...
Site and Console have no business-logic coverage floor. Their mandatory gates are route ownership,
static output, navigation, i18n, and Worker boundary contracts; lowering Core coverage is never an
accepted way to absorb the split.
