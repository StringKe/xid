...
async function cleanupPasswordResetSmokeOrganization(organizationId) {
...
`,
  ...
  printResult('PASS', 'production password reset cleanup', `org=${organizationId}`)
}

async function seedPasswordResetSmokeOrganization() {
  const now = Date.now()
...
`,
  ...
  const organizationId = `org_smoke_pwreset_${crypto.randomUUID()}`
...
export async function runProductionPasswordResetSmoke() {
  ...
  try {
    if (!providedUrl) {
      organizationId = await seedPasswordResetSmokeOrganization()
    ...
    try {
      await cleanupPasswordResetSmokeOrganization(organizationId)
    } catch (cleanupError) {
