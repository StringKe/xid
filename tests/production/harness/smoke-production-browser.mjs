...
async function ensureSmokeManager(userId, tenantId) {
  const now = Date.now()
  const memId = `mem_smoke_${crypto.randomUUID()}`
  const mgrId = `mgr_smoke_${crypto.randomUUID()}`
  await d1(
    `
...
async function seedSmokeUser(targetEmail) {
  ...
  const tenantId = 'org_1dbae1c2-4d13-410f-99d2-69378c588594'
  const now = Date.now()
  const userId = `user_smoke_${crypto.randomUUID()}`
  const emailId = `eml_smoke_${crypto.randomUUID()}`
  await d1(
    `
INSERT INTO users
...
DELETE FROM manager_assignments WHERE user_id = ${sqlString(userId)};
DELETE FROM memberships WHERE user_id = ${sqlString(userId)};
DELETE FROM verification_tokens WHERE user_id = ${sqlString(userId)};
DELETE FROM password_reset_tokens WHERE user_id = ${sqlString(userId)};
DELETE FROM user_emails WHERE user_id = ${sqlString(userId)};
DELETE FROM users WHERE id = ${sqlString(userId)};
`,
    'cleanup production browser smoke user',
  )
  printResult('PASS', 'production browser smoke cleanup', `user=${userId}`)
}

async function cleanupPasswordSmokeOrganization(organizationId, targetEmail) {
...
FROM user_emails
WHERE tenant_id = ${sqlString(organizationId)}
  AND email = ${sqlString(targetEmail)}
LIMIT 1;
`,
    'load production password smoke user for cleanup',
  )
  const userId = rows[0]?.user_id
  await d1(
    `
DELETE FROM sessions WHERE tenant_id = ${sqlString(organizationId)};
DELETE FROM password_history WHERE tenant_id = ${sqlString(organizationId)};
DELETE FROM passwords WHERE tenant_id = ${sqlString(organizationId)};
DELETE FROM password_reset_tokens WHERE tenant_id = ${sqlString(organizationId)};
DELETE FROM passkey_credentials WHERE tenant_id = ${sqlString(organizationId)};
DELETE FROM backup_codes WHERE tenant_id = ${sqlString(organizationId)};
DELETE FROM mfa_factors WHERE tenant_id = ${sqlString(organizationId)};
...
`,
  ...
  printResult(
    ...
    `org=${organizationId}${userId ? ` user=${userId}` : ''}`,
  )
}
...
`,
  ...
  printResult('PASS', 'production platform organization cleanup', `org=${organizationId}`)
}
...
`,
  ...
  const organizationId = `org_smoke_password_${crypto.randomUUID()}`
...
`,
  ...
  const instanceId = instanceRows[0]?.id
  if (!instanceId) throw new Error('production instance missing')
  const organizationId = `org_smoke_${crypto.randomUUID()}`
...
async function checkPlatformOrganizationMutation(cookie) {
  ...
  try {
    ...
    await cleanupSmokeOrganization(organizationId)
  }
}
...
export async function runProductionBrowserSmoke() {
  ...
  let cookie = null
  let passwordOrganizationId = null
  let primaryError = null
  try {
    let me = null
    await seedSmokeUser(smokeEmail)
    await cleanupSmokeUser(passwordSmokeEmail)
    passwordOrganizationId = await seedPasswordSmokeOrganization()
    await withChrome(async (page) => {
      ...
      await checkActiveOrganization(page, cookie, me)
      await checkOrgConsoleRoutes(page)
      await checkSdkBrowserIntegration(page, cookie, me)
      await checkMfaProviderGate(page, cookie)
      const passwordResult = await checkPasswordSignInFlow(page, passwordOrganizationId)
      await checkMfaSelfServiceFlow(page, passwordOrganizationId, passwordResult.userId)
      const mfaLogin = await setupTotpSelfService(
        page,
        passwordOrganizationId,
      ...
      await cleanupMfaSelfService(passwordOrganizationId, passwordResult.userId)
      const passkeyResult = await checkPasskeyRegistrationAndSignInFlow(
        page,
        passwordOrganizationId,
        passwordResult.userId,
      )
      await signOutSmokeUser(passkeyResult.cookie)
      await signOutSmokeUser(mfaResult.cookie)
    })
    await checkPlatformOrganizationMutation(cookie)
    ...
    try {
      await signOutSmokeUser(cookie)
      await cleanupSmokeUser(smokeEmail)
      await cleanupPasswordSmokeOrganization(passwordOrganizationId, passwordSmokeEmail)
      await cleanupSmokeUser(passwordSmokeEmail)
    } catch (cleanupError) {
