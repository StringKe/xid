...
async function signOutEnterpriseSsoSession(cookie) {
  ...
  if (res.status !== 200) {
    throw new Error(`enterprise sso sign out failed http=${res.status} body=${text}`)
  }
  const after = await fetchText('/v1/me', { cookie })
  if (after.res.status === 200) {
    throw new Error('enterprise sso session still authenticates after sign out')
  }
  printResult('PASS', 'enterprise sso sign out', `http=${res.status} me=${after.res.status}`)
}

async function establishEnterpriseSsoSession(samlResponse, session) {
  ...
  if (samlResponse) {
    ...
    session.cookie = await postSamlAcs(connectionId, samlResponse, relayState)
    await verifyEnterpriseSession(session.cookie, connectionId, expectedOrgId)
    return
  }
  ...
  session.cookie = await fetchOidcCallback(callbackUrl)
  await verifyEnterpriseSession(session.cookie, connectionId, expectedOrgId)
}
...
export async function runProductionEnterpriseSsoSmoke() {
  ...
  const unregisterSignals = registerCleanupSignalHandlers(() =>
    signOutEnterpriseSsoSession(session.cookie),
  )
  try {
    await establishEnterpriseSsoSession(samlResponse, session)
    ...
    const { failures } = await runCleanupSteps([
      {
        name: 'enterprise sso session sign out',
