...
명령은 `native:verify`를 호출하는데, `XID_NATIVE_SDK_PLATFORM`을 설정하지 않으면 CI 워크플로 계약만
검증하므로 네이티브 툴체인이 필요하지 않습니다. GitHub Actions는 검증만 하고 배포는 하지
...
가져다 사용하는 방식입니다. 대신 CI가 정확성을 강제합니다. 6개의 `native-*` job(그중 3개는 matrix로
확장되어 13개 플랫폼 전부를 포함)이 `tests/native-sdk-contract.test.mjs`의 계약을 기준으로 각 언어의
자체 테스트 스위트를 실행합니다. pull request에서는 `dorny/paths-filter` job이 브랜치가 건드린 SDK
디렉터리와 `ci.yml`, 계약 파일 자체로 대상을 좁히고, `main`으로의 모든 push에서는 13개 전부를
실행합니다. 로컬에서 하나만 실행하려면 `XID_NATIVE_SDK_PLATFORM=go pnpm run native:verify`를
사용하십시오. 플랫폼별 성숙도는
[`docs/sdks/platform-matrix.md`](docs/sdks/platform-matrix.md)에 있습니다.

## 문서

독자별로 경로를 안내하는 [`docs/README.md`](docs/README.md)에서 시작하십시오. 설계 및 운영 문서는
대부분 중국어이며, 프로토콜 매트릭스와 일부 SDK 레퍼런스 페이지는 영어입니다.
