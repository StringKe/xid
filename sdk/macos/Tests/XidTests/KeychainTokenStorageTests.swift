...
final class KeychainTokenStorageTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        storage = KeychainTokenStorage(service: "dev.xid.sdk.test.macos")
    ...
    func testLoadNonExistentKeyReturnsNil() throws {
        let result = try storage.load(key: "xid.test.nonexistent.macos")
        XCTAssertNil(result)
    }
    ...
    func testDeleteNonExistentDoesNotThrow() {
        XCTAssertNoThrow(try storage.delete(key: "xid.test.gone.macos"))
    }
}
