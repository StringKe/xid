...
valider le contrat du workflow CI et n'exige aucune toolchain native. GitHub Actions vérifie mais
ne déploie jamais ; le déploiement en production part de Cloudflare Workers Builds sur le compte du
propriétaire du dépôt. Voir [`CONTRIBUTING.md`](CONTRIBUTING.md) pour le workflow propre à chaque
domaine.
...
consomment depuis les sources. La CI en garantit la correction autrement : six jobs `native-*`,
dont trois développés en matrice et couvrant les treize plateformes, exécutent la suite de tests
propre à chaque langage à partir du contrat défini dans `tests/native-sdk-contract.test.mjs`. Sur
une pull request, un job `dorny/paths-filter` les restreint aux répertoires de SDK touchés par la
branche, plus `ci.yml` et le fichier de contrat lui-même ; chaque push sur `main` exécute les
treize. Pour en lancer un en local : `XID_NATIVE_SDK_PLATFORM=go pnpm run native:verify` ; la
maturité par plateforme est documentée dans
[`docs/sdks/platform-matrix.md`](docs/sdks/platform-matrix.md).

## Documentation

Commencez par [`docs/README.md`](docs/README.md), qui oriente selon le profil du lecteur. La
plupart des documents de conception et d'exploitation sont en chinois ; les matrices de protocoles
et plusieurs pages de référence des SDK sont en anglais.
