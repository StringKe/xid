...
expanded, covering all thirteen platforms -- run each language's own test suite from the contract in
`tests/native-sdk-contract.test.mjs`. On a pull request a `dorny/paths-filter` job narrows them to
the SDK directories the branch touched, plus `ci.yml` and the contract file itself; every push to
`main` runs all thirteen. Run one locally with `XID_NATIVE_SDK_PLATFORM=go pnpm run native:verify`;
per-platform maturity is in
