...
export function initSearch(config: SearchConfig): SearchInstance {
  const { input, resultsContainer, emptyState, provider, labels, onNavigate } = config;
  ...
  async function ensureInitialized(): Promise<boolean> {
    ...
    try {
      ...
      emptyState.textContent = `${labels.buildOnly}.`;
      return false;
    }
  }
  ...
  async function runSearch(query: string): Promise<void> {
    ...
    emptyState.textContent = `${labels.searching}...`;
    clearResults();
    ...
    try {
      ...
      clearResults();
      emptyState.style.display = "";
      emptyState.textContent = `${labels.unavailable}.`;
    }
  }
  ...
  function handleInput(): void {
    ...
    debounceTimer = setTimeout(() => {
      ...
      if (!query) {
        ...
        clearResults();
        emptyState.style.display = "";
        emptyState.textContent = `${labels.typeToSearch}...`;
        return;
      }
...
mount("[data-search-dialog]", (root) => {
  ...
  const search = initSearch({
    ...
    provider,
    labels: {
      buildOnly: dialog.dataset.labelBuildOnly ?? "",
      noResults: dialog.dataset.labelNoResults ?? "",
      searching: dialog.dataset.labelSearching ?? "",
      typeToSearch: dialog.dataset.labelTypeToSearch ?? "",
      unavailable: dialog.dataset.labelUnavailable ?? "",
    },
