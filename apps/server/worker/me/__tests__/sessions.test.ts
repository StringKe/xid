...
describe('POST /v1/me/sessions/revoke-all', () => {
  ...
  it('SessionDO revoke-all-except 返回 500 时 fail closed:D1 不落 revoked 且返回 500', async () => {
    const row = sessionRow({ id: 's_other', user_id: 'u_42' })
    const db = makeFakeD1({ sessions: [row] })
    const names: string[] = []
    const env = { DB: db, SESSION_REVOCATION: makeFakeSessionNs(names, 500) } as unknown as Env
    const app = buildApp({
      register: registerMeSessionsRoutes,
      session: makeSession({ sessionId: 's_current', userId: 'u_42' }),
    })
...
describe('DELETE /v1/me/sessions/:id', () => {
  ...
  it('SessionDO revoke 返回 500 时 fail closed:D1 不落 revoked 且返回 500', async () => {
    const row = sessionRow({ id: 's_other', user_id: 'u_42' })
    const db = makeFakeD1({ sessions: [row] })
    const names: string[] = []
    const env = { DB: db, SESSION_REVOCATION: makeFakeSessionNs(names, 500) } as unknown as Env
