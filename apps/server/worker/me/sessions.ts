...
import { sessionDoRevokeAllExcept, sessionDoStub } from '../lib/session'
...
// per-user SessionDO 单条撤销(命中签发时同一实例 key=session:{userId},见 v1/sessions.ts revokeInDO)。
...
app.delete('/:id', async (c) => {
  ...
  await revokeInDO(c.env, session.userId, id)
