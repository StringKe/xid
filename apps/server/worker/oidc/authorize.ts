...
function localErrorPage(
  ...
  return renderProtocolErrorPage(c, { status: 400, error, description })
}
...
async function stashAndRedirect(
  ...
  await stub.fetch('https://oauth-flow-do/store', {
    method: 'POST',
    body: JSON.stringify({
...
async function consumeStashedAuthorize(
  ...
  const ns = c.env.OAUTH_STATE
  const stub = ns.get(ns.idFromName(`authz:${ctx.tenantId}:${authzRequestId}`))
  const res = await stub.fetch('https://oauth-flow-do/consume', {
    method: 'POST',
    body: JSON.stringify({ state: authzRequestId }),
  })
  if (res.status !== 200) return null
  const body = (await res.json()) as { record?: { pendingParams?: RawParams } }
...
async function runAuthorize(c: Context<XidHonoEnv>, params: RawParams): Promise<Response> {
  ...
  if (!parResult.ok) return localErrorPage(c, parResult.error, parResult.description)
