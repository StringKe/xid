...
async function claimRequestObjectJti(
  ...
  const stub = ns.get(ns.idFromName(`jar:${ctx.tenantId}`))
  const key = `${clientId}#${jti}`
  const claimed = await stub.fetch('https://oauth-flow-do/claim', {
    method: 'POST',
    body: JSON.stringify({ state: key, ttlMs: JAR_REQUEST_OBJECT_TTL_SEC * 1000 }),
  })
  return claimed.status === 201
}
...
export async function resolveRequestObject(input: {
  ...
  const claimError = checkClaims(payload, {
    ctx,
  ...
  const fresh = await claimRequestObjectJti(input.c, input.client.clientId, payload.jti as string)
  if (!fresh) return fail('request object jti replayed')
