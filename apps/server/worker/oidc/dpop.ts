...
async function claimJti(c: Context<XidHonoEnv>, htu: string, jti: string): Promise<boolean> {
  const key = `${htu}#${jti}`
  const stub = dpopStub(c)
  const claimed = await stub.fetch('https://oauth-flow-do/claim', {
    method: 'POST',
    body: JSON.stringify({ state: key, ttlMs: DPOP_PROOF_WINDOW_SEC * 1000 }),
  })
