...
async function claimAssertionJti(
  ...
  const stub = ns.get(ns.idFromName(`pkjwt:${ctx.tenantId}`))
  const key = `${clientId}#${jti}`
  const claimed = await stub.fetch('https://oauth-flow-do/claim', {
    method: 'POST',
    body: JSON.stringify({ state: key, ttlMs: PRIVATE_KEY_JWT_WINDOW_SEC * 1000 }),
  })
...
async function verifyPrivateKeyJwt(input: {
  ...
  const fresh = await claimAssertionJti(input.c, input.client.clientId, payload.jti as string)
  if (!fresh) return { ok: false, error: clientError(401, 'client_assertion jti replayed') }
