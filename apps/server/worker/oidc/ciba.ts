...
async function claimCibaRedemption(
  ...
  const stub = ns.get(ns.idFromName(`ciba:${tenantId}`))
  const claimed = await stub.fetch('https://oauth-flow-do/claim', {
    method: 'POST',
    body: JSON.stringify({ state: authReqId, ttlMs: CIBA_AUTH_REQ_TTL_SEC * 1000 }),
  })
...
export async function grantCiba(
  ...
  if (!(await claimCibaRedemption(tc.c, ctx.tenantId, authReqId))) {
    return fail('invalid_grant', 'auth_req_id already consumed')
  }
