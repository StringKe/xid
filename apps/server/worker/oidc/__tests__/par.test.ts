...
import { registerParRoutes, REQUEST_URI_PREFIX } from '../par'
...
describe('/par round-trip', () => {
  ...
  // DO 故障与"request_uri 不存在"合并成 400 会让 RP 认定自己参数错而不重试,运维也看不到 5xx。
  it('ParStore consume 返回 500 -> 本地 server_error 而不是 invalid_request', async () => {
    const { ctx } = await buildTestTenant()
    const env = makeEnv({
      DB: makeFakeD1({ applications: [appRow()] }),
    ...
    const res = await app.request(
      `https://acme.xid.dev/authorize?${new URLSearchParams({
        client_id: CLIENT_ID,
        request_uri: `${REQUEST_URI_PREFIX}do-down`,
      }).toString()}`,
      {},
      env,
    )
  ...
  // ParStore handleConsume 对已过期条目回 400 + error=expired_token,这是 RP 的问题不是故障。
  it('ParStore consume 返回 400 expired_token -> 本地 invalid_request', async () => {
    const { ctx } = await buildTestTenant()
    const env = makeEnv({
      DB: makeFakeD1({ applications: [appRow()] }),
      PAR_STORE: makeFakeDoNs((path) =>
        path === '/consume'
          ? Response.json(
              { error: 'expired_token', error_description: 'request_uri has expired' },
              { status: 400 },
            )
          : new Response('{}', { status: 404 }),
      ),
    ...
    const res = await app.request(
      `https://acme.xid.dev/authorize?${new URLSearchParams({
        client_id: CLIENT_ID,
        request_uri: `${REQUEST_URI_PREFIX}expired`,
      }).toString()}`,
      {},
      env,
    )
  ...
  // /consume 收到坏 body 时 ParStore 回 400 invalid_request,那是调用方契约违规不是 RP 的错。
  it('ParStore consume 返回 400 invalid_request -> 本地 server_error', async () => {
    const { ctx } = await buildTestTenant()
    const env = makeEnv({
      DB: makeFakeD1({ applications: [appRow()] }),
      PAR_STORE: makeFakeDoNs((path) =>
        path === '/consume'
          ? Response.json(
              { error: 'invalid_request', error_description: 'Missing required field: requestUri' },
              { status: 400 },
            )
          : new Response('{}', { status: 404 }),
      ),
    ...
    const res = await app.request(
      `https://acme.xid.dev/authorize?${new URLSearchParams({
        client_id: CLIENT_ID,
        request_uri: `${REQUEST_URI_PREFIX}bad-call`,
      }).toString()}`,
      {},
      env,
    )
  ...
  it('ParStore consume 返回非 JSON -> 本地 server_error 而不是空参数放行', async () => {
    const { ctx } = await buildTestTenant()
    const env = makeEnv({
      DB: makeFakeD1({ applications: [appRow()] }),
      PAR_STORE: makeFakeDoNs((path) =>
        path === '/consume' ? new Response('not json') : new Response('{}', { status: 404 }),
      ),
      OAUTH_STATE: oauthStateNoop(),
    })
    const app = appWith(ctx, session())
    ...
    const res = await app.request(
      `https://acme.xid.dev/authorize?${new URLSearchParams({
        client_id: CLIENT_ID,
        request_uri: `${REQUEST_URI_PREFIX}malformed-json`,
      }).toString()}`,
      {},
      env,
    )
  ...
  // client_id 一致(躲过 10.3 的一致性校验),但 redirect_uri 不是字符串:
  // 修复前这种 body 会被整体放行进 evaluateAuthorize。
  it('ParStore consume 返回非字符串参数 -> 本地 server_error', async () => {
    const { ctx } = await buildTestTenant()
    const env = makeEnv({
      DB: makeFakeD1({ applications: [appRow()] }),
      PAR_STORE: makeFakeDoNs((path) =>
    ...
    const res = await app.request(
      `https://acme.xid.dev/authorize?${new URLSearchParams({
        client_id: CLIENT_ID,
        request_uri: `${REQUEST_URI_PREFIX}malformed-body`,
      }).toString()}`,
      {},
      env,
    )
