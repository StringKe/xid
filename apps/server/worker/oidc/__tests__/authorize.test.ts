...
describe('/authorize', () => {
  ...
  it('无 session + 暂存写入失败 -> server_error 错误页且不跳登录页', async () => {
    const { ctx } = await buildTestTenant()
    const { app, env } = setup({ applications: [appRow()] }, ctx, null)
    env.OAUTH_STATE = makeFakeDoNs((path) =>
      path === '/store'
        ? new Response('store failed', { status: 500 })
