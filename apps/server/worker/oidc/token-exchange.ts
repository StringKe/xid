...
export async function grantDeviceCode(tc: TokenContext): Promise<GrantResult> {
  ...
  if (!res.ok) {
    ...
    return fail(
      (body.error ?? 'invalid_grant') as XidError['code'],
      body.error_description ?? 'device flow error',
    )
  }
  const approved = (await res.json()) as { userId: string; scopes: string[]; clientId: string }
  // 兑换侧再校验一次 client 绑定(纵深防御,DO 已校验)。
  if (approved.clientId !== tc.clientId) {
    return fail('invalid_grant', 'device_code not bound to this client')
  }
  return issueDeviceTokens(tc, approved)
}
