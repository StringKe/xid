...
// insert ... returning 由 D1 fake 回读 table 末行,签发类用例预置这一行。
function issuedRow(id: string, authenticatedAt: Date, expiresAt: Date): SessionRow {
  return {
    id,
    tenant_id: 't_1',
    user_id: 'u_1',
...
function makeFakeSessionNs(opts: {
  ...
  alwaysThrowAction?: string
  generation?: number
  rejectAdd?: boolean
  responseStatusByAction?: Record<string, number>
  // 200 但 body 形状不符契约(空 body / 非 number generation / 缺 accepted)。
  rawBodyByAction?: Record<string, unknown>
  ...
  const stub = {
    fetch: async (url: string, init?: RequestInit) => {
      ...
      // DO 整个不可达:抛异常而非返回错误状态码,重试耗尽后仍抛。
      if (opts.alwaysThrowAction === action) {
        const error = new Error('DO unreachable') as Error & { retryable: boolean }
      ...
      // 故障响应体不带任何否定信号:只有真正检查 HTTP status 的实现才会拒绝(否则静默放行)。
      const status = opts.responseStatusByAction?.[action] ?? 200
      if (status !== 200) return Response.json({}, { status })
      if (opts.rawBodyByAction && action in opts.rawBodyByAction) {
        return Response.json(opts.rawBodyByAction[action])
      }
...
// fail closed 用例:handler 抛错时 Hono 会吞成 500 响应,靠 console.error 反推抛了什么
// 会把断言绑在 harness 的实现细节上。这里把异常原样抓出来交给用例直接断言,
// 同时保留 Set-Cookie 观察点(cookie 必须没被设/没被清)。
async function runExpectingThrow(
  env: Env,
  handler: (c: Context<XidHonoEnv>) => Promise<unknown>,
  tenant: TenantContext = TENANT,
): Promise<{ error: unknown; setCookie: string | null }> {
  const app = new Hono<XidHonoEnv>()
  ...
  const res = await app.request('http://acme.example.test/', {}, env)
  if (!threw) throw new Error('handler was expected to throw but returned normally')
  return { error: captured, setCookie: res.headers.get('set-cookie') }
}
...
describe('issueSession', () => {
  ...
  it('SessionDO add 返回 500 时 fail closed:不签发 cookie', async () => {
    const table: SessionRow[] = []
    const env = makeEnv(
      makeFakeD1({ sessions: table, users: [activeUser()] }),
      makeFakeSessionNs({ active: true, responseStatusByAction: { add: 500 } }),
    )
    const expiresAt = new Date(Date.now() + 3600_000)
    const authenticatedAt = new Date()

    const { error, setCookie } = await runExpectingThrow(env, async (c) => {
      table.push(issuedRow('s_do_500', authenticatedAt, expiresAt))
      return issueSession(c, {
        sessionId: 's_do_500',
        userId: 'u_1',
        authenticatedAt,
  ...
  it('SessionDO generation 返回 500 时 fail closed:不写 D1 也不签发 cookie', async () => {
    const table: SessionRow[] = []
    const capture = { insertParams: [] as unknown[][] }
    const env = makeEnv(
      makeFakeD1({ sessions: table, users: [activeUser()] }, capture),
      makeFakeSessionNs({ active: true, responseStatusByAction: { generation: 500 } }),
    )

    const { error, setCookie } = await runExpectingThrow(env, (c) =>
      issueSession(c, {
        sessionId: 's_gen_500',
        userId: 'u_1',
        authenticatedAt: new Date(),
  ...
  // 200 + {} 曾被 body.value?.accepted !== false 判成"接受":fencing 是防 revoke-all 之后
  // 旧签发复活会话的唯一判定点,契约漂移必须当故障,不能降级成放行。
  it('SessionDO add 返回空 body 时按故障处理,不签发 cookie', async () => {
    const table: SessionRow[] = []
    const env = makeEnv(
      makeFakeD1({ sessions: table, users: [activeUser()] }),
      makeFakeSessionNs({ active: true, rawBodyByAction: { add: {} } }),
    )
    const expiresAt = new Date(Date.now() + 3600_000)
    const authenticatedAt = new Date()

    const { error, setCookie } = await runExpectingThrow(env, async (c) => {
      table.push(issuedRow('s_add_empty', authenticatedAt, expiresAt))
      return issueSession(c, {
        sessionId: 's_add_empty',
        userId: 'u_1',
        authenticatedAt,
        expiresAt,
      })
  ...
  it('SessionDO add 的 accepted 非布尔时按故障处理,不签发 cookie', async () => {
    const table: SessionRow[] = []
    const env = makeEnv(
      makeFakeD1({ sessions: table, users: [activeUser()] }),
      makeFakeSessionNs({
        active: true,
        rawBodyByAction: { add: { ok: true, value: { accepted: 'yes' } } },
      }),
    )
    const expiresAt = new Date(Date.now() + 3600_000)
    const authenticatedAt = new Date()
    ...
    const { error, setCookie } = await runExpectingThrow(env, async (c) => {
      table.push(issuedRow('s_add_bad', authenticatedAt, expiresAt))
      return issueSession(c, {
        sessionId: 's_add_bad',
        userId: 'u_1',
        authenticatedAt,
  ...
  it('SessionDO add 返回 Result 错误分支时按故障处理,不签发 cookie', async () => {
    const table: SessionRow[] = []
    const env = makeEnv(
      makeFakeD1({ sessions: table, users: [activeUser()] }),
      makeFakeSessionNs({
        active: true,
        rawBodyByAction: { add: { ok: false, error: { code: 'server_error' } } },
      }),
    )
    const expiresAt = new Date(Date.now() + 3600_000)
    const authenticatedAt = new Date()
    ...
    const { error, setCookie } = await runExpectingThrow(env, async (c) => {
      table.push(issuedRow('s_add_err', authenticatedAt, expiresAt))
      return issueSession(c, {
        sessionId: 's_add_err',
        userId: 'u_1',
        authenticatedAt,
  ...
  // generation 静默回退 0 会让后续 add 拿错误基准比对,把契约违规吞成一次"撤销"。
  it('SessionDO generation 缺字段时按故障处理,不写 D1 也不签发 cookie', async () => {
    const table: SessionRow[] = []
    const capture = { insertParams: [] as unknown[][] }
    const env = makeEnv(
      makeFakeD1({ sessions: table, users: [activeUser()] }, capture),
      makeFakeSessionNs({ active: true, rawBodyByAction: { generation: {} } }),
    )

    const { error, setCookie } = await runExpectingThrow(env, (c) =>
      issueSession(c, {
        sessionId: 's_gen_missing',
        userId: 'u_1',
        authenticatedAt: new Date(),
        expiresAt: new Date(Date.now() + 3600_000),
      }),
  ...
  it('SessionDO generation 非 number 时按故障处理,不写 D1 也不签发 cookie', async () => {
    const table: SessionRow[] = []
    const capture = { insertParams: [] as unknown[][] }
    const env = makeEnv(
      makeFakeD1({ sessions: table, users: [activeUser()] }, capture),
      makeFakeSessionNs({ active: true, rawBodyByAction: { generation: { generation: '3' } } }),
    )

    const { error, setCookie } = await runExpectingThrow(env, (c) =>
      issueSession(c, {
        sessionId: 's_gen_string',
        userId: 'u_1',
        authenticatedAt: new Date(),
        expiresAt: new Date(Date.now() + 3600_000),
      }),
...
describe('readSessionById', () => {
  ...
  it('DO is-active 返回 500 时 fail closed 为 null', async () => {
    const token = 'tok_do_500'
    const row = await seedRow(token)
    const env = makeEnv(
      makeFakeD1({ sessions: [row], users: [activeUser()] }),
      makeFakeSessionNs({ active: true, responseStatusByAction: { 'is-active': 500 } }),
    )
  ...
  // DO 不可达必须与"伪造 cookie"得到同样的 null,否则故障窗口内的响应差异就是 cookie 有效性 oracle。
  it('DO is-active 整个不可达时返回 null 而不是把异常抛成 500', async () => {
    const token = 'tok_do_unreachable'
    const row = await seedRow(token)
    const env = makeEnv(
      makeFakeD1({ sessions: [row], users: [activeUser()] }),
      makeFakeSessionNs({ active: true, alwaysThrowAction: 'is-active' }),
    )
...
describe('revokeSession', () => {
  ...
  it('SessionDO revoke 返回 500 时 fail closed:D1 已 revoked 且不清 cookie', async () => {
    ...
    const env = makeEnv(
      makeFakeD1([row]),
      makeFakeSessionNs({ active: true, responseStatusByAction: { revoke: 500 } }),
    )
    const session: SessionData = {
      sessionId: 's_500',
      userId: 'u_500',
    ...
    const { error, setCookie } = await runExpectingThrow(env, (c) => revokeSession(c, session))
