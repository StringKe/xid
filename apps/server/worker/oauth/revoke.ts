...
// 形状层:token 必填非空 string;hint 限 RFC7009 两个值(未知 hint 直接拒)。
...
async function revokeAccessToken(
  ...
  // denylist 读写失败必须 fail closed:静默 200 会让 RP 相信 token 已失效,
  // 而 token 仍能通过验签继续访问资源。RFC7009 的 200 只覆盖"不认识该 token",不覆盖存储故障。
  try {
    ...
    throw new AppError('server_error', { cause: err })
  }
}
