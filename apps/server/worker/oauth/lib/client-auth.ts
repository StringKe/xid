...
async function claimAssertionJti(
  ...
  const claimed = await stub.fetch('https://oauth-flow-do/claim', {
    method: 'POST',
    body: JSON.stringify({ state: key, ttlMs: 300 * 1000 }),
  })
...
async function authenticateByAssertion(
  ...
  const fresh = await claimAssertionJti(c, row.clientId, jti)
  if (!fresh) return clientErr('client_assertion jti replayed')
  return { ok: true, value: rowToClient(row) }
}
