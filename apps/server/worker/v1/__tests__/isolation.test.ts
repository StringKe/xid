...
// status 可调:响应体不带否定信号,只有检查 HTTP status 的实现才会 fail closed。
function makeFakeSessionNs(names: string[], status = 200): DurableObjectNamespace {
  const stub = {
...
describe('v1 shared asynchronous work', () => {
  ...
  // A broken RateLimitStore must not let a bulk invitation batch through, even when the failure
  // body carries no negative signal at all (status 500 plus allowed:true).
  it('fails closed on invitation quota when the RateLimitStore returns a 500', async () => {
    const fetch = vi.fn(async () => Response.json({ allowed: true }, { status: 500 }))
    const env = {
      RATE_LIMITER: {
    ...
    await expect(checkInvitationRateLimit(c, 30)).rejects.toMatchObject({
      code: 'server_error',
      httpStatus: 500,
    })
  })
...
describe('v1 sessions 撤销:命中 per-user SessionDO(idFromName=session:{userId})', () => {
  ...
  it('revoke 单条遇 SessionDO 500 时 fail closed:D1 不落 revoked', async () => {
    const { token, row: apiKey } = await makeApiKeyRow('t_1')
    const session = { id: 's_1', tenant_id: 't_1', user_id: 'u_42', status: 'active' }
    const db = makeFakeD1({ api_keys: [apiKey], sessions: [session] })
    const names: string[] = []
    const env = asUnknown<Env>({ DB: db, SESSION_REVOCATION: makeFakeSessionNs(names, 500) })
    const app = buildApp(registerSessionsRoutes)
  ...
  it('revoke_all 遇 SessionDO 500 时 fail closed:D1 不落 revoked', async () => {
    const { token, row: apiKey } = await makeApiKeyRow('t_1')
    const session = { id: 's_1', tenant_id: 't_1', user_id: 'u_42', status: 'active' }
    const db = makeFakeD1({
      api_keys: [apiKey],
