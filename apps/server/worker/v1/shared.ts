...
// count is the batch size and MUST reach the DO, otherwise a 30-invitation request would consume
// one unit of the hourly quota instead of 30. checkRateLimitStore owns the fail-closed contract:
// an unreachable or malformed RateLimitStore raises server_error rather than passing the batch.
export async function checkInvitationRateLimit(
  ...
  const decision = await checkRateLimitStore(c.env, key, INVITATION_BULK_POLICY, { count })
  if (!decision.allowed) {
