...
function rateLimitStub(env: Env, key: string): DurableObjectStub {
...
export async function checkRateLimit(
  ...
  const stub = rateLimitStub(env, key)
...
export async function reserveRateLimitWindows(
  ...
  const stub = rateLimitStub(env, reservationKey)
  const res = await stub.fetch('https://rate-limit/reserve', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ windows }),
  })
  const result = (await res.json()) as { allowed: boolean }
  if (!res.ok || !result.allowed) throw new AppError('rate_limited')
}
