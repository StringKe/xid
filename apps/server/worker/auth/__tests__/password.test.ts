...
describe('hashPassword + verifyPassword', () => {
  ...
  // 存量 hash 兼容 fixture:由 @noble/hashes 1.4.0 按同一编码规则产出。
  // 换 KDF 库或参数导致存量密码全部登录失败时,这条先红。
  it('验证既有 argon2id fixture(跨 KDF 库版本不变)', async () => {
    const stored =
      '$argon2id$v=19$m=65536,t=3,p=1$AwoRGB8mLTQ7QklQV15lbA$UBgn0t679HfoQKD08agMPuwwSboKff_TNkKZ4_gFsHU'
    expect(await verifyPassword('LegacyStoredPass123!', stored, 'argon2id', PEPPER)).toBe(true)
    expect(await verifyPassword('WrongStoredPass123!', stored, 'argon2id', PEPPER)).toBe(false)
  }, 30000)
})
