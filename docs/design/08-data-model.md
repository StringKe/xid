...
Tenant -> TenantSigningKey(已废弃保留表)
User -> Session -> Token
```
...
| TenantSigningKey   | 已废弃保留表,不参与默认签发                                |
...
以下章节是 `packages/db`(Drizzle schema)与 `packages/types`(TypeScript 类型)的**唯一真相源**。每个核心实体给出完整字段表 + 索引声明 + 外键 ON DELETE 策略。字段名/物理类型确定后实现层不得偏离;字段增删改先改本章再改实现。当前共 60 张 D1 表(与 `packages/db/src/schema`、`packages/db/drizzle` migration 一致);device_codes / par_requests 是 DO 内逻辑结构,不占表数。
...
> tenant_id 语义:XID 的"租户"=顶层 Organization(见 02 章层级,Instance -> Organization)。多数业务表 `tenant_id` 指向顶层 org 的 id;org 级细分实体再带 `org_id`(子 org / active org)。平台级表(Instance/PlatformAdmin/License/FeatureFlag(KV))不带 tenant_id,走独立管理路径(见 tenant-isolation rule)。
...
> 加密字节布局固定:`access_token_ciphertext = version(1B) || iv(12B) || ciphertext(变长) || tag(16B)`。KEK 是 account 级 `env.KEK`(Workers Secrets),本表无独立 kek_version 列,version=1 硬编码嵌入密文首字节(worker/auth/social-providers.ts encryptToken);独立 kek_version 列只存在于 instance_signing_keys / cert_store / tenant_signing_keys(见 16.2-16.4)。明文 token 永不入库。
...
| verification_method     | text            | NOT NULL                                           | `'dns_txt'`         | `dns_txt`(`idx-verify=<token>`)/`https_file`(见 04 章 5)     |
...
索引:`UNIQUE(org_id)`、`INDEX(tenant_id)`、`INDEX(tenant_id, status)`。SsoProfile(单次认证结果 idp_id/claims)不持久化(瞬时),如需审计走 audit_events;DirectoryUser 双向绑定见 16.4。
...
> 决策:SAML 私钥与 OIDC 签名私钥采用**相同信封加密结构但分表存**。CertStore 把 iv / ciphertext / tag **拆三个 blob 字段**(便于按字段读取与 KEK 解密),不用单 JSON blob。InstanceSigningKey(16.3)与 TenantSigningKey(16.4)同结构。
...
### 16.4 tenant_signing_keys(已废弃保留表)

该表是已废弃保留表。托管生产默认 `issuer = https://xid.dev` 时不从本表签发,也不让 admin/app org 成为默认 issuer 签发方。字段结构与 `instance_signing_keys` 相同,但隔离键为 `tenant_id`。
...
> metering_user_index:`(tenant_id TEXT, user_id TEXT, int_id INTEGER)`,`UNIQUE(tenant_id, user_id)`。残留表,当前无读写;MAU 去重走 MeteringDO storage(见上)。
...
### 17.7 licenses(残留表,平台级)

XID 是 MIT 许可,不做任何许可密钥校验。本表是残留结构,`packages/db/src/schema/operations.ts` 仍保留定义,但代码中无任何读写路径。自托管方若要在 XID 之上叠加自有商业授权,可复用该结构;XID 内核不依赖它。

| 字段                    | 类型            | 约束                                           | 默认   | 说明                                |
| ----------------------- | --------------- | ---------------------------------------------- | ------ | ----------------------------------- |
| id                      | text            | PK                                             | nanoid |                                     |
| instance_id             | text            | NOT NULL, FK -> instances.id ON DELETE cascade | --     |                                     |
| license_key             | text            | NOT NULL, UNIQUE                               | --     | 授权标识,XID 内核不校验             |
| tier                    | text            | NOT NULL                                       | --     | `free`/`starter`/`pro`/`enterprise` |
| domain                  | text            | null                                           | null   | 绑定域                              |
| valid                   | integer boolean | NOT NULL                                       | `1`    | 状态标记                            |
| expiry                  | integer ts_ms   | null                                           | null   | 到期时间                            |
| created_at / updated_at | integer ts_ms   | NOT NULL                                       | 见 9.3 |                                     |

索引:`UNIQUE(license_key)`、`INDEX(instance_id)`。
...
| tenant_signing_keys 私钥       | 同上拆三字段                                                                                    | 已废弃保留表复用相同密钥材料结构                                                   |
