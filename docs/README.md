...
`docs/protocols/` 把每条协议能力绑定到标准来源、支持等级、代码路径、测试路径和证据等级。"支持"在这里有严格定义,不是营销词。
...
| 部署后怎么验证跑通了  | [protocols/conformance-plan.md](./protocols/conformance-plan.md) |
...
`docs/design/` 是产品设计真相源,记录每个子系统**为什么**这样设计。改实现前先读对应章节;改设计先改那里。
