# Shibboleth inbound SAML runbook

## Console steps

1. Open **Console -> Organization -> Inbound enterprise SSO**.
2. Click **Add Shibboleth template** to pre-fill metadata URL, attribute mapping, and JIT defaults.
...
- Provider: `Shibboleth`; direction: `inbound`.
...
- BLOCKED evidence: record missing Shibboleth admin access, isolated test tenant, callback domain, test user, assignment, or signing material. Do not record secrets, complete assertions, or personal data.
